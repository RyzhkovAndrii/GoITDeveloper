...
FROM (
	...
	FROM (
		SELECT company_id, customer_id, SUM(cost - project_expenses) project_profit
		FROM companies_projects
		JOIN customers_projects USING (project_id)
		JOIN projects USING (project_id)
		JOIN (
			SELECT pd.project_id, SUM(salary) project_expenses
	...
	SELECT com.company_name, cus.customer_name, SUM(cost - project_expenses) project_profit
	FROM companies_projects 
	JOIN customers_projects USING (project_id)
	JOIN projects USING (project_id)
